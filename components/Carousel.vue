...
let changeInSlides: MutationObserver, viewbox: HTMLElement | null, slider: HTMLElement | null, slides: HTMLElement[], currSlide: HTMLElement | null, prevSlide: HTMLElement | null, trackerCont: HTMLElement | null, prevBtn: HTMLElement | null, nextBtn: HTMLElement | null, initialized: boolean;
...
function init() {
    slides = [...(slider?.querySelectorAll(`:scope > .cs-slide`) || [])].filter(el => !el.matches(`[data-creator='${tmp.uniqueId}']`)) as HTMLElement[];
    if (!slides[0]) return;
    ...
    rawSlides.forEach((el) => {
        ...
        el.style.marginRight = `${tmp.spaceBetween}px`;
    });
...
function dist(e: MouseEvent | TouchEvent) {
    if (settings.direction === 'vertical') return e.type.indexOf('touch') > -1 ? (e as TouchEvent).touches[0].clientY : (e as MouseEvent).clientY;
    return e.type.indexOf('touch') > -1 ? (e as TouchEvent).touches[0].clientX : (e as MouseEvent).clientX;
}
...
function gestureEnd(e: MouseEvent | TouchEvent) {
    ...
    if (slider?.classList.contains('swiping') && tmp.coordChange) {
        ...
        if (tmp.gestureTarget instanceof HTMLElement && tmp.gestureTarget.closest('[data-lightbox], [data-find-lightbox-content]')) {
            let toggler = tmp.gestureTarget.closest('[data-lightbox], [data-find-lightbox-content]') as HTMLElement;
            let lightboxElem = document.querySelector(`#${toggler.getAttribute('data-target')}`) as HTMLElement;
