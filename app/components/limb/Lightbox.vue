  ...
  if (bb.newCoords.change) {
    ...
    if (
      ...
      if (bb.newCoords.zoom === 1) {
        ...
        if (bb.newCoords.x > 0) {
          if (prevSlide)
            prevSlide.style.transform = `translate(${bb.newCoords.x - bb.sliderRect!.width
              }px, 0px)`;
        } else if (nextSlide)
          nextSlide.style.transform = `translate(${bb.newCoords.x + bb.sliderRect!.width
            }px, 0px)`;
      }
...
<template>
  <div ref="lightbox" :id="props.id" role="dialog" aria-modal="true" aria-label="Media lightbox dialog" class="lightbox"
    ...
    <div class="icon-bar lb-control inverted menu">
      ...
      <div class="r-aligned items">
        <div class="resp-down-hidden items">
          <button class="item active as-icon gallery-switch" v-tooltip:aria.unblocking aria-label="Show all slides">
            <Icon mode="svg" name="material-symbols:apps" class="icon" />
          </button>
          <button v-if="settings.slideshow" class="item as-icon slideshow" :class="{ disabled: vbb.slidesNo < 2 }"
            v-tooltip:aria.unblocking aria-label="Slideshow Play or Pause">
          ...
          <button class="item as-icon fullscreen-switch dd-close" v-tooltip:aria.unblocking
        ...
        <LimbDropdown class="item as-icon resp-up-hidden" aria-label="More options"
        ...
        <div class="drop menu">
          ...
          <div role="menuitem" v-if="settings.slideshow" class="item slideshow dd-close"
        ...
        <button class="item as-icon pic-only" v-tooltip:aria.unblocking aria-label="Picture only view">
          <Icon mode="svg" name="material-symbols:image-outline" class="icon" />
        </button>
        <button class="item as-icon exit-lightbox" v-tooltip:aria.unblocking aria-label="Exit lightbox">
    ...
    <div class="slides" :data-slidesNo="settings.gallery ? vbb.slidesNo : null">
      <figure v-if="vbb.slidesNo" ref="slide" class="slide" v-for="(slide, index) in vbb.slidesData"
        :data-gallery-ref="index + 1" :data-cont-type="slide.type" :data-cont-id="slide.contentId">
        ...
        <div class="content">
          ...
          <iframe v-else-if="slide.type === 'youtube'"
            :src="`https://www.youtube-nocookie.com/embed/${slide.videoId}?enablejsapi=1&playsinline=1`" frameborder="0"
            allowfullscreen></iframe>
          <iframe v-else-if="slide.type === 'unknown'" :src="slide.url" frameborder="0" allowfullscreen></iframe>
      ...
      <button aria-label="Previous Slide" :disabled="vbb.currSlideNo <= 1" class="lb-control c-prev">
        <Icon mode="svg" name="material-symbols:chevron-left" class="icon" />
      </button>
      <button aria-label="Next Slide" :disabled="vbb.currSlideNo === vbb.slidesNo" class="lb-control c-next">
        <Icon mode="svg" name="material-symbols:chevron-right" />
      </button>
    </div>
    <LimbIScroller :options="{
      autoSetup: false,
