...
function clickToOpenLightbox(e: MouseEvent) {
  ...
  let toggler = target.closest(`${settings.toggler}[data-target="${props.id}"]`) as HTMLElement;
  if (!toggler || target.closest(settings.toExcuseToggler)) return;
  triggerViewOf(toggler);
}
async function init() {
  let newTogglers = (
    [...document.querySelectorAll(`[data-target="${props.id}"]`)] as HTMLElement[]
  ).filter((el) => el.matches("[data-lightbox]") || el.matches("[data-find-lightbox-content]"));
...
function toolbarControls(e: MouseEvent) {
  ...
  if (target.closest(".item.zoom-in, .item.zoom-out")) {
    ...
    bb.newCoords.zoom = Math.min(
      settings.maxZoom,
      target.closest(".item.zoom-in") ? bb.newCoords.zoom + 0.5 : bb.newCoords.zoom - 0.5,
    );
    reAdjustSlide();
  }
  // Gallery-view toggler
    ...
    if (lightbox.value!.classList.contains("pic-only")) {
      ...
      [...toolbar.querySelectorAll(":scope .item.gallery-switch")].forEach((el) =>
    ...
    utils.afterNextRepaint(async () => {
      reAdjustSlide();
      if (lightbox.value!.classList.contains("show-gallery")) {
        // delay set activeView of gallery to get accurate values
        await utils.delay(settings.transitionDuration);
        utils.triggerEvent(
          gallery,
          "activeView",
          gallery.querySelector(`:scope .thumbnail[data-gallery-ref="${vbb.currSlideNo}"]`),
        );
        resizeIframe();
      }
    });
  ...
  // Fullscreen functionality
    ...
    if (document.fullscreenElement === lightbox.value!) {
      ...
      [...toolbar.querySelectorAll(":scope .item.fullscreen-switch")].forEach((el) =>
        el.classList.add("active"),
      );
    }
  }
...
async function resizeFunc() {
  ...
  if (lightbox.value!.classList.contains("show-gallery")) {
    // delay set activeView of gallery to get accurate values
    await utils.delay(settings.transititonDuration);
    utils.triggerEvent(
      gallery,
      "activeView",
      gallery.querySelector(`:scope .thumbnail[data-gallery-ref="${vbb.currSlideNo}"]`),
...
function zoomToFit(coords: { x: number; y: number; zoom?: number }) {
  ...
  if (coords.zoom === undefined) return { x: pointOnContentX, y: pointOnContentY };
...
function exitOnWrapperClick(e: MouseEvent) {
  let target = e.target as HTMLElement;
  if (
    (settings.closeOnWrapperClick && !target.closest(settings.toBeConsidered)) ||
...
function reAdjustSlide() {
  if (bb.newCoords.zoom > 1) {
    ...
    let xMax = Math.abs((currPic.clientWidth * bb.newCoords.zoom - currContent.clientWidth) / 2);
    let yMax = Math.abs((currPic.clientHeight * bb.newCoords.zoom - currContent.clientHeight) / 2);

    bb.newCoords.x =
      Math.abs(bb.newCoords.x) > xMax ? Math.sign(bb.newCoords.x) * xMax : bb.newCoords.x;
    bb.newCoords.y =
      Math.abs(bb.newCoords.y) > yMax ? Math.sign(bb.newCoords.y) * yMax : bb.newCoords.y;
...
onMounted(async () => {
  ...
  toolbar = lightbox.value!.querySelector(":scope > .icon-bar.lb-control") as HTMLElement;
  gallery = lightbox.value!.querySelector(":scope > .gallery-view") as HTMLElement;
  ...
  async (value) => {
    if (value) {
      ...
      bb.inOutTimeout = setTimeout(() => {
        utils.triggerEvent(lightbox.value!, "lightboxReady", settings);
...
function getContent(el: HTMLElement) {
    ...
    if (ref !== null) {
      try {
        ...
        if (hostname.includes("youtube.com") || hostname.includes("youtu.be")) {
          ...
          content.videoId = parsedUrl.searchParams.get("v") || pathname.split("/").pop();
  ...
  if (el.hasAttribute("data-caption")) content.caption = el.getAttribute("data-caption") ?? "";
...
async function update(newSlideNo: number = vbb.currSlideNo) {
  newSlideNo = newSlideNo < 1 ? 1 : newSlideNo > vbb.slidesNo ? vbb.slidesNo : newSlideNo;
  ...
  [...toolbar.querySelectorAll(":scope .zoom-out")].forEach((el) => el.classList.add("disabled"));
...
function dist(e: TouchEvent | MouseEvent) {
  if ("touches" in e && e.touches.length > 1) {
    ...
    return {
      x: "touches" in e ? (e.touches[0]?.clientX ?? 0) : (e as MouseEvent).clientX,
      y: "touches" in e ? (e.touches[0]?.clientY ?? 0) : (e as MouseEvent).clientY,
...
function gestureStart(e: TouchEvent | MouseEvent) {
  ...
  bb.maybeCaption = !!target.closest(".caption");
  ...
  if (!target.closest(".caption.full-text")) {
    ...
    if (currSlide.getAttribute("data-cont-type") === "image") {
      ...
      if (tapLength < 300 && tapLength > 100 && !("touches" in e && e.touches.length > 1)) {
        if (bb.newCoords.zoom === 1) {
          const coords = dist(e);
          zoomToFit({
            x: coords.x,
...
function gestureMove(e: TouchEvent | MouseEvent) {
  ...
  if (Math.abs(coordDiffX) > 5 || Math.abs(coordDiffY) > 5) bb.newCoords.change = true;

  if (bb.newCoords.change) {
    // Pinch Zooming in action
    if ("touches" in e && e.touches.length === 2 && currContent.classList.contains("zooming")) {
      ...
      if (bb.newCoords.zoom === 1) {
        ...
        if (bb.newCoords.x > 0) {
          ...
          nextSlide.style.transform = `translate(${bb.newCoords.x + bb.sliderRect!.width}px, 0px)`;
      }
      // Zoom-dragging in action
      else
        currContent.style.transform = `translate(${bb.newCoords.x}px, ${bb.newCoords.y}px) scale(${bb.newCoords.zoom})`;
    } else if (bb.maybeCaption && e.type === "touchmove") bb.captionTogglePrevented = true;
  }
}
function gestureEnd(e: TouchEvent | MouseEvent) {
  ...
  if (currContent.matches(".zooming, .zoom-dragging")) {
    ...
    if (
      ((bb.newCoords.change && Math.abs(changeExt) > currSlide.clientWidth / 20) ||
        (Math.abs(changeExt) > currSlide.clientWidth / 40 &&
          new Date().getTime() - bb.gestureTime < 300)) &&
      ((bb.newCoords.x < 0 && bb.moveDir === 1) || (bb.newCoords.x > 0 && bb.moveDir === -1))
    ) {
      if (bb.newCoords.x < 0) {
        update(vbb.currSlideNo + 1);
        stopSlideshow();
...
function startSlideshow() {
  async function startProgressBar() {
    ...
    utils.setCSS(progressBar, {
      width: "100%",
      transition: `width ${settings.slideshowDuration - 200}ms ease 0s;`,
    });
  }
  ...
  if (currSlide.getAttribute("data-cont-type") === "video") (currPic as HTMLVideoElement).pause();
  else if (currSlide.getAttribute("data-cont-type") === "youtube")
    vbb.slidesData
      ?.find((el) => el.contentId === currSlide.getAttribute("data-cont-id"))
      ?.youtubeRef?.pauseVideo();
...
<template>
  ...
  >
    ...
    <div class="icon-bar lb-control inverted menu">
      <div class="xhover status item">{{ vbb.currSlideNo }} / {{ vbb.slidesNo }}</div>
      <div class="r-aligned items">
        <div class="resp-down-hidden items">
          ...
          <button class="item as-icon zoom-in" v-tooltip:aria.unblocking aria-label="Zoom in">
            <Icon mode="svg" name="material-symbols:zoom-in" class="icon" />
          </button>
          <button class="disabled item zoom-out" v-tooltip:aria.unblocking aria-label="Zoom out">
    ...
    <div class="slides" :data-slidesNo="settings.gallery ? vbb.slidesNo : null">
      ...
      <figure v-else class="lb-placeholder" style="text-align: center">
        ...
        <figcaption class="font-semibold huge">No Lightbox Element Present</figcaption>
      </figure>
      <button
