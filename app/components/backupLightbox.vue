...
async function init() {
  let newTogglers = [
    ...
    (el) =>
      ...
      el.hasAttribute('data-find-lightbox-content')
  );
  ...
  (async function () {
    if (bb.youtubeAPIState === 1) {
      ...
      slides.value
        .filter((el) => el.getAttribute('data-cont-type') === 'youtube')
        .forEach((el) => {
...
function update(newSlideNo = vbb.currSlideNo, initialize) {
  ...
  [...toolbar.value.querySelectorAll(':scope .zoom-out')].forEach((el) =>
...
function panelClickFunc(e) {
  if (
    (settings.closeOnWrapperClick &&
      !e.target.closest(settings.toBeConsidered)) ||
    (settings.dismissible && e.target.closest(settings.dismisser))
  )
    vbb.showLightbox = false;
}
...
function toolbarControls(e) {
  ...
  if (e.target.closest('.item.zoom-in')) {
    stopSlideshow();
    bb.newCoords.zoom = bb.newCoords.zoom + 0.5;
    reAdjustSlide();
    [...toolbar.value.querySelectorAll(':scope .item.zoom-out')].forEach((el) =>
      el.classList.remove('disabled')
    );
    resizeIframe();
  } else if (e.target.closest('.item.zoom-out')) {
    bb.newCoords.zoom =
      bb.newCoords.zoom - 0.5 < 1 ? 1 : bb.newCoords.zoom - 0.5;
    stopSlideshow();
  ...
  // Gallery-view toggler
    ...
    if (lightbox.value.classList.contains('pic-only')) {
      [...toolbar.value.querySelectorAll(':scope .item.pic-only')].forEach(
        (el) => el.classList.remove('active')
      );
      lightbox.value.classList.remove('pic-only');
      [
        ...toolbar.value.querySelectorAll(':scope .item.gallery-switch'),
  ...
  // Fullscreen functionality
    ...
    if (document.fullscreenElement === lightbox.value) {
      ...
      [
        ...toolbar.value.querySelectorAll(':scope .item.fullscreen-switch'),
      ].forEach((el) => el.classList.add('active'));
    }
  }
  // Pic-only Funtionality
  else if (e.target.closest('.item.pic-only')) {
    [...toolbar.value.querySelectorAll(':scope .pic-only')].forEach((el) =>
...
function gallerySwitch(e) {
  if (e.target.closest('.thumbnail')) {
    update(
      Number(e.target.closest('.thumbnail').getAttribute('data-gallery-ref'))
    );
    stopSlideshow();
  }
}
function resizeFunc() {
  resizeIframe();
  bb.newCoords.w = slider.value.getBoundingClientRect().width; // update for swiping purpose;
  if (bb.newCoords.zoom > 1) reAdjustSlide();

  if (lightbox.value.classList.contains('show-gallery')) {
    // delay set activeView of gallery to get accurate values
    let activeThumbnail = gallery.querySelector(
      `:scope .thumbnail[data-gallery-ref="${vbb.currSlideNo}"]`
    );
...
function wheelToZoom(e) {
  ...
  delta > 0
    ? (bb.newCoords.zoom = bb.newCoords.zoom * 1.2)
    : (bb.newCoords.zoom = bb.newCoords.zoom / 1.2);
...
function reAdjustSlide() {
  if (bb.newCoords.zoom > 1) {
    ...
    bb.yMax =
      Math.abs(
        (currPic.clientHeight * bb.newCoords.zoom - currContent.clientHeight) /
          2
      ) | 0;
    bb.newCoords.x =
      Math.abs(bb.newCoords.x) > bb.xMax
        ? Math.sign(bb.newCoords.x) * bb.xMax
        : bb.newCoords.x;
    bb.newCoords.y =
      Math.abs(bb.newCoords.y) > bb.yMax
        ? Math.sign(bb.newCoords.y) * bb.yMax
        : bb.newCoords.y;
  }
  currContent.style.transform = `translate(${bb.newCoords.x}px, ${bb.newCoords.y}px) scale(${bb.newCoords.zoom})`;
}
function dist(e) {
  if (e.touches?.length === 2) {
    let [t1, t2] = e.touches;
    let rects = currContent.getBoundingClientRect();
    let pRect = currSlide.getBoundingClientRect();
...
function gestureStart(e) {
  ...
  if (!e.target.closest('.caption.full-text')) {
    ...
    if (e.type === 'touchstart' && e.touches.length === 2) {
      ...
      bb.newCoords.w = currContent.getBoundingClientRect().width;
      bb.newCoords.h = currContent.getBoundingClientRect().height;
...
function gestureMove(e) {
  ...
  bb.endCoords = dist(e);
  let coordDiff = {
    x: bb.endCoords.x - bb.startCoords.x,
    y: bb.endCoords.y - bb.startCoords.y,
  };

  if (Math.abs(coordDiff.x) > 5 || Math.abs(coordDiff.y) > 5)
  ...
  if (
    ...
    let zoomFactor = bb.endCoords.z / bb.startCoords.z;
    bb.newCoords.zoom = bb.initCoords.zoom * zoomFactor;

    // bb.newCoords.x =
    //   (bb.newCoords.x - window.innerWidth / 2 + bb.endCoords.x) * zoomFactor -
    //   (bb.endCoords.x - window.innerWidth / 2);
    // bb.newCoords.y =
    //   (bb.newCoords.y - window.innerHeight / 2 + bb.endCoords.y) * zoomFactor -
    //   (bb.endCoords.y - window.innerHeight / 2);
    // bb.newCoords.x = bb.initCoords.x + coordDiff.x - (bb.initCoords.x * (zoomFactor - 1));
    // bb.newCoords.y = bb.initCoords.y + coordDiff.y - (bb.initCoords.y * (zoomFactor - 1));

    bb.newCoords.x = (((bb.startCoords.cx * zoomFactor) - bb.startCoords.px)) + coordDiff.x;
    bb.newCoords.y = (((bb.startCoords.cy * zoomFactor) - bb.startCoords.py)) + coordDiff.y;
    // console.log(eRect.x - pRect.x);
    console.log(bb.newCoords.x, bb.newCoords.y);
    ...
    bb.newCoords.x = bb.initCoords.x + coordDiff.x;
    bb.newCoords.y = bb.initCoords.y + coordDiff.y;
    bb.newCoords.change = true;
...
function startSlideshow() {
  settings.slidePlayer = setInterval(() => {
    ...
    setTimeout(
      () =>
        progressBar.value.setAttribute(
          'style',
  ...
  [...toolbar.value.querySelectorAll(':scope .item.slideshow')].forEach((el) =>
    el.classList.add('active')
  );
    ...
    >
      ...
      >
        ...
        <div v-else class="lb-placeholder" style="text-align: center">
          ...
          <div class="font-semibold huge">No Lightbox Element Present</div>
        </div>
        <div
          ref="prevBtn"
