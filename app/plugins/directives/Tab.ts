...
export default {
  mounted(el: HTMLElement, binding: DirectiveBinding) {
    function tabClick(e: Event | "on" | undefined) {
      const tabs = [...el.querySelectorAll(":scope [data-tab]")] as HTMLElement[];
      ...
      let currTab = tabs.find((el) =>
        typeof e === "object" ? el.contains(e.target as Node) : el.classList.contains("active"),
      );
      ...
      if (currTab) {
        ...
        if (currTab.getAttribute("data-tab")) {
          let currPages = pages.filter(
            (el) => currTab.getAttribute("data-tab") === el.getAttribute("data-tab"),
          );
